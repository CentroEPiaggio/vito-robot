...
class dataPublisher
{
  private:
    ...
    typedef message_filters::sync_policies::ApproximateTime<phase_space::PhaseSpaceMarkerArray ,sensor_msgs::PointCloud2,geometry_msgs::TransformStamped,geometry_msgs::TransformStamped> MySyncPolicy;
  ...
  dataPublisher(ros::NodeHandle nh) : nh_(nh), priv_nh_("~"),sub_markers_readings(nh,nh.resolveName("/phase_space_markers"),1),sub_camera_readings(nh,"/camera/depth_registered/points",1),tf_readings_wrist(nh,"/phase_space_world_to_/bracelet",1),tf_readings_star(nh,"/phase_space_world_to_/star",1),sync(MySyncPolicy(100),sub_markers_readings,sub_camera_readings,tf_readings_wrist,tf_readings_star)
  { 
...
void dataPublisher::callback(const MarkerPtr& markers,const scenePtr& scene, const transPtr& t_world_wrist, const transPtr& t_world_star )
{
